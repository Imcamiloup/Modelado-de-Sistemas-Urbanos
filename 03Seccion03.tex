...
\begin{itemize}
    ...
    \item \( K(U) \): Capacidad de carga, que depende de la huella urbana \( U \), es decir, cuánta población puede ser soportada en función del espacio y recursos disponibles.
...
$$
$$
\lambda_1 = 3 \rho, \quad \lambda_2 = \sqrt{\gamma^2 + \bar{\alpha} \gamma}, \quad \lambda_3 = - \sqrt{\gamma^2 + \bar{\alpha} \gamma}
$$
...
\]
donde \(\alpha_W,\beta_W,\gamma_W,\delta_W\) cuantifican los efectos de cada variable sobre \(W(t)\).
Para el componente poblacional, previamente se estableció
\[
\frac{dP}{dt} \;=\; rP\left(1-\frac{P}{K} \right)
\]
donde \( r \) y \( K \) son parámetros de la dinámica poblacional.
...
\label{eq:subPW}
\begin{cases}
\displaystyle \frac{dP}{dt} \;=\; rP\left(1 - \frac{P}{\bar K}\right),\\[6pt]
...
\[
W^\star_1 \;=\; -\frac{1}{\gamma_W}\Big(\alpha_W P + \beta_W(\bar U+\bar E) - \delta_W \bar F\Big).
\]
\vspace{-0.25em}
\begin{itemize}
...
Al ejecutar este bloque de código, se resuelve la ecuación diferencial con los parámetros óptimos y se definen las funciones \( P(t) \) y \( dP_(t) \) para su uso posterior.
...
Implementamos la función para calcular el error (residuo) entre el modelo y los datos observados. Esta función calcula el residuo (error) al comparar la población, la vegetación y el entorno modelados con los datos observados.
...
Aquí se muestra la función de optimización que minimiza el error cuadrático medio (MSE) entre el bienestar urbano simulado y los datos observados. El proceso de optimización se realiza utilizando el algoritmo L-BFGS-B. A continuación, se muestra el bloque de código relevante para este proceso.
