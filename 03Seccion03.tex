...
El enfoque adoptado se basa en la teor´ıa de sis-
temas din´amicos no lineales y el an´alisis cualitativo de ecuaciones diferenciales ordinarias
(EDOs
  ...
  \[
    \frac{dP}{dt} = rP \left( 1 - \frac{P}{K(U)} \right) + I(t) + bP - dP
  \]
  ...
  \begin{itemize}
    ...
    \item \( K(U) \): Capacidad de carga, que depende de la huella urbana \( H \), es decir, cuánta población puede ser soportada en función del espacio y recursos disponibles.
  ...
  Este sistema de ecuaciones permite modelar el crecimiento poblacional teniendo en cuenta los efectos de la migración, los nacimientos, las muertes y las limitaciones impuestas por el espacio disponible (reflejado en \( K(U) \)).
  ...
  \begin{itemize}
    ...
    \item \(k_1 U(t) - k_2 E(t)\) la capacidad de carga efectiva, dependiente de la huella urbana \(U(t)\) y limitada por la estructura ecológica \(E(t)\).
  ...
  \begin{itemize}
    ...
    \item \(\bar{\beta}\): Coeficiente de preservación ecológica relacionado con el área protegida \( E \).
      ...
      Modelo basado en el modelo SIR (epidemiológico). Suponemos que es un sistema cerrado, esto es, \( D(t) + S(t) + U(t) = C \) para todo \( t \geq 0 \), donde \( C \) es el área total (en este caso de Bogotá).

      \[
        \frac{dD}{dt} = -\alpha D(t) S(t)
      \]

      \[
        \frac{dS}{dt} = \alpha D(t) S(t) - \gamma S(t)
      \]

      \[
        \frac{dU}{dt} = \gamma S(t)
      \]

      donde \( \alpha, \gamma > 0 \).

      No obstante, dado que \( U(t) \) no aparece en ninguna de las ecuaciones del sistema, vemos que esta puede determinarse a partir de \( S(t) \) y no aporta en nada a la dinámica del sistema. Por tanto, este puede reducirse a:
      ...
      \[
        ...
        \theta_W E(t)  + \gamma_W W(t) - \delta_W F(t) + \lambda_U U(t) + \lambda_E E(t)
      \]
      ...
      \begin{itemize}
        ...
        \item Los parámetros \( \lambda_U \) y \( \lambda_E \) controlan la influencia directa de \( U(t) \) y \( E(t) \) en el bienestar urbano.
      \end{itemize}
      ...
      \subsubsection{Población (\( P(t) \))}

      La \textit{población} sigue modelándose como un proceso logístico clásico, teniendo en cuenta la capacidad de carga urbana. Sin embargo, se introduce una interacción explícita entre la \textit{desigualdad social} \( F(t) \) y la tasa de crecimiento de la población, lo que refleja cómo las desigualdades pueden ralentizar el crecimiento poblacional al limitar el acceso a servicios, educación y salud. La ecuación para la población \( P(t) \) queda modificada como sigue:

      \[
        \frac{dP}{dt} = rP(t)\left(1 - \frac{P(t)}{K(U, E, F)}\right) + \gamma_F F(t)
      \]

      donde \( \gamma_F \) es un parámetro que mide el impacto de la desigualdad social sobre la tasa de crecimiento de la población. Esta modificación es coherente porque un aumento en la desigualdad reduce las oportunidades para los sectores más desfavorecidos, lo que afecta las tasas de natalidad, acceso a recursos y, en última instancia, el crecimiento poblacional.

      \subsubsection{Huella Urbana (\( U(t) \))}

      La \textit{huella urbana} \( U(t) \), que describe el área urbanizada, tiene una relación directa con la población \( P(t) \) y la demanda de viviendas \( V(t) \), pero también está limitada por la estructura ecológica \( E(t) \). Se ha introducido una nueva interacción entre \( U(t) \) y \( E(t) \) de manera que la expansión urbana ahora se ve afectada negativamente por la cantidad de áreas ecológicas:

      \[
        \frac{dU}{dt} = \alpha_P P(t) + \beta_V V(t) - \gamma_E E(t)
      \]

      Aquí, \( \gamma_E \) es un parámetro que indica cómo la estructura ecológica limita la expansión de la huella urbana. Un mayor número de áreas ecológicas disponibles restringe la urbanización, favoreciendo un crecimiento urbano más sostenible y respetuoso con el medio ambiente. Este ajuste es coherente con los principios de planificación urbana sostenible, donde la conservación de las áreas naturales debe ser prioritaria frente a la expansión urbana descontrolada.

      \subsubsection{Estructura Ecológica (\( E(t) \))}

      La \textit{estructura ecológica} \( E(t) \), que se refiere a las áreas protegidas y los ecosistemas naturales de la ciudad, se ve afectada por la expansión de la huella urbana y por la capacidad de gestión del territorio. En este modelo, \( E(t) \) interactúa con \( U(t) \), por lo que su valor disminuye a medida que la ciudad se expande:

      \[
        \frac{dE}{dt} = -\delta_U U(t)
      \]

      Esto refleja cómo la urbanización disminuye la cantidad de espacio dedicado a la naturaleza, lo que es coherente con la dinámica observada en ciudades en expansión. A medida que la ciudad crece, las áreas ecológicas se ven reducidas, lo que contribuye a la pérdida de biodiversidad y la degradación ambiental.

      \subsubsection{Desigualdad Social (\( F(t) \))}

      Finalmente, la \textit{desigualdad social} \( F(t) \), que mide la disparidad en el acceso a recursos y servicios, afecta directamente tanto al bienestar urbano como al crecimiento poblacional. La desigualdad puede generar tensiones sociales, disminuir el acceso a servicios básicos y, por lo tanto, reducir el bienestar de la población. La ecuación para \( F(t) \) se introduce con un término de retroalimentación negativa sobre \( W(t) \):

      \[
        \frac{dF}{dt} = -\alpha_F F(t) + \beta_F W(t)
      \]

      Este término refleja cómo una mayor desigualdad genera una retroalimentación negativa sobre el bienestar urbano, mientras que una mejora en el bienestar puede contribuir a la reducción de la desigualdad, mediante políticas de redistribución y acceso a recursos.

      \subsubsection{Impacto de los Cambios en el Modelo}

      Los ajustes realizados en el modelo tienen un impacto significativo en las dinámicas de las variables del sistema. El refuerzo de la interacción entre \( U \), \( E \) y \( F \) hace que estas variables ya no se traten como factores externos, sino como elementos clave que influyen directamente en el bienestar urbano, la sostenibilidad ecológica y la evolución de la población. La relación ajustada entre estas variables permite modelar con mayor precisión los efectos de las políticas públicas y las dinámicas sociales sobre el crecimiento urbano y la calidad de vida de los habitantes.

      \subsubsection{Problema Identificado: Degeneración del Sistema}

      \textbf{Consecuencia matemática:} Al plantear el sistema autónomo con $\frac{dU}{dt} = 0$ y $\frac{dE}{dt} = 0$, se generó una degeneración del Jacobiano, evidenciada por:

      \begin{itemize}
        \item Matriz Jacobiana de rango deficiente ($\text{rank}(\mathbf{J}) < 5$)
        \item Aparición de valores propios nulos ($\lambda_4 = \lambda_5 = 0$)
        \item Puntos fijos con componentes físicamente inadmisibles (ej. $U = -3{,}322{,}390$)
      \end{itemize}

      \textbf{Justificación teórica:} Las ecuaciones $\frac{dU}{dt} = 0$ y $\frac{dE}{dt} = 0$ admiten infinitos puntos de equilibrio en las coordenadas $(U, E)$, convirtiendo al conjunto de equilibrios en una variedad bidimensional en lugar de puntos aislados.

      \subsubsection{Decisión Metodológica: Tratamiento de Variables Exógenas}

      Para resolver la degeneración, adoptamos el siguiente enfoque:

      \begin{quote}
        \textit{Las variables $U(t)$ y $E(t)$ son tratadas como \textbf{forzamientos externos} con evolución temporal lenta (escala de tiempo $\tau_{\text{ecológica}} \gg \tau_{\text{social}}$), y se fijan en sus valores promedio normalizados para el análisis de equilibrio.}
      \end{quote}

      \textbf{Fundamentación física:}
      \begin{itemize}
        \item La huella urbana ($U$) cambia gradualmente (infraestructura física)
        \item La infraestructura ecológica ($E$) evoluciona en décadas (parques, zonas protegidas)
        \item En contraste, $P$, $F$ y $W$ responden a dinámicas sociales más rápidas
      \end{itemize}

      \subsubsection{Cálculo de Valores de Forzamiento}

      Dado un conjunto de datos históricos $\{U_i, E_i\}_{i=1}^{n}$, normalizados mediante:

      \begin{equation}
        \tilde{U}_i = \frac{U_i - \mu_U}{\sigma_U}, \quad \tilde{E}_i = \frac{E_i - \mu_E}{\sigma_E}
      \end{equation}

      Los valores de forzamiento se determinan como:

      \begin{equation}
        U_{\text{fijo}} = \frac{1}{n}\sum_{i=1}^{n} \tilde{U}_i \approx 0, \quad E_{\text{fijo}} = \frac{1}{n}\sum_{i=1}^{n} \tilde{E}_i \approx 0
      \end{equation}

      \textbf{Resultado numérico obtenido:}
      \begin{align}
        U_{\text{fijo}} &= 0.000000\\
        E_{\text{fijo}} &= 0.000000
      \end{align}
...
\end{verbatim}
      ...
      El ajuste se llevó a cabo mediante mínimos cuadrados, utilizando polinomios de distinto grado según la complejidad observada en cada serie temporal:

      \begin{itemize}
        \item Grado 3 para \textbf{Población} \(P(t)\) y \textbf{Huella urbana} \(U(t)\).
        \item Grado 4 para \textbf{Bienestar urbano} \(W(t)\) y \textbf{Desigualdad} \(F(t)\).
        \item Grado 10 para \textbf{Estructura ecológica} \(E(t)\), con el fin de capturar mejor la variabilidad observada en la serie asociada a la infraestructura ecológica.
      \end{itemize}

      En todos los casos, la variable independiente \(t\) representa el año calendario (por ejemplo, \(t = 2012, 2013, \dots, 2024\)). A continuación se presentan las ecuaciones ajustadas, expresadas en términos de \(t\).

      A continuación, presentamos las ecuaciones ajustadas para cada variable:

      \subsubsection{Estructura Ecológica}

      La ecuación ajustada para la Estructura Ecológica es de grado 10:
      ...
      La ecuación ajustada para el Bienestar Urbano es de grado 4:

      \[
        W(t) = 0.0002 t^4 - 0.0048 t^3 + 0.0345 t^2 - 0.0956 t + 0.0368
      \]

      \subsubsection{Desigualdad}

      La ecuación ajustada para la Desigualdad es de grado 4:

      \begin{equation}
        \begin{aligned}
          F(t) \;=\;& -10126177734.3653
          + 20082636.9226\,t
          - 14935.7269\,t^{2} \\
          &+ 4.9368\,t^{3}
          - 0.0006\,t^{4}.
      ...
      La ecuación ajustada para la Población es de grado 3:
      ...
      \subsubsection{Huella Urbana}

      La ecuación ajustada para la Huella Urbana es de grado 3:

      \begin{equation}
        \begin{aligned}
          U(t) \;=\;& -30439939673.6538
          + 45332622.4182\,t
          - 22503.8252\,t^{2}
          + 3.7238\,t^{3}.
        \end{aligned}
      ...
      \subsubsection{Gráficos del Ajuste}

      A continuación, se muestran los gráficos de los datos originales (puntos azules) y las curvas ajustadas (curvas rojas) para cada variable:

      \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{Huella_Urbana_ajuste.png}
        \caption{Ajuste polinómico de la Huella Urbana (Grado 3)}
      \end{figure}
      ...
      \begin{figure}[H]
        ...
        \begin{minipage}{0.45\textwidth}
          \centering
          \includegraphics[width=\linewidth]{Estructura_Ecologica_ajuste.png}
          \caption{Ajuste polinómico del Área de Estructura Ecologica (Grado 3)}
        \end{minipage}
        \hfill
        \begin{minipage}{0.45\textwidth}
          \centering
          \includegraphics[width=\linewidth]{Desigualdad_ajuste.png}
          \caption{Ajuste polinómico de la Desigualdad (Grado 3)}
        \end{minipage}
      \end{figure}
      ...
      \begin{figure}[H]
        ...
        \begin{minipage}{0.45\textwidth}
          \centering
          \includegraphics[width=\linewidth]{Estructura_Ecologica_ajuste.png}
          \caption{Ajuste polinómico del Bienestar Urbano(Grado 4)}
        \end{minipage}
        \hfill
        \begin{minipage}{0.45\textwidth}
          \centering
          \includegraphics[width=\linewidth]{Bienestar_ajuste.png}
          \caption{Ajuste polinómico del Bienestar Urbano (Grado 3)}
        \end{minipage}
      \end{figure}

      \subsubsection{Interpretación de los Resultados}

      Los parámetros ajustados para las funciones polinómicas son los siguientes:

      \begin{itemize}
        \item Para la \textbf{Estructura Ecológica} (Grado 10):
          \[
            E(t) = -0.0001 t^{10} + 0.0012 t^9 - 0.0115 t^8 + 0.0745 t^7 - 0.2906 t^6 + 0.7231 t^5 - 1.0431 t^4 + 0.9942 t^3 - 0.5023 t^2 + 0.1267 t - 0.0003
          \]
        \item Para el \textbf{Bienestar Urbano} (Grado 4):
          \[
            W(t) = 0.0002 t^4 - 0.0048 t^3 + 0.0345 t^2 - 0.0956 t + 0.0368
          \]
        \item Para la \textbf{Desigualdad} (Grado 4):
          \[
            D(t) = -0.0025 t^4 + 0.0204 t^3 - 0.0568 t^2 + 0.0326 t - 0.0059
          \]
        \item Para la \textbf{Población} (Grado 3):
          \[
            P(t) = -0.93 t^3 + 5631.41 t^2 - 1.14 \times 10^7 t + 7.66 \times 10^9
          \]
        \item Para la \textbf{Huella Urbana} (Grado 3):
          \[
            H(t) = 3.72 t^3 - 2.25 \times 10^4 t^2 + 4.53 \times 10^7 t - 3.04 \times 10^{10}
          \]
      \end{itemize}

      Estos ajustes nos permiten describir la evolución temporal de cada variable de manera continua, lo cual facilita el análisis de la dinámica del sistema en su conjunto.
...
\end{minted}
      ...
      \noindent En los puntos fijos
      $$
      P^* = \frac{K \pm \sqrt{K^2 + \frac{4K I}{r}}}{2},
      $$
      la entrada diagonal evaluada es
      $$
      r\left(1-\frac{2P^*}{K}\right) \;=\; \mp\, r\sqrt{1+\frac{4I}{rK}}.
      $$

      Por tanto, al evaluar el Jacobiano en cada punto fijo se obtiene
      $$
      J(P^*,I^*) =
      \begin{bmatrix}
        \mp\, r\sqrt{1+\frac{4I^*}{rK}} & 1 \\[4pt]
        0 & 0
      \end{bmatrix},
      $$
      y los autovalores son
      $$
      \lambda_1 = 0,\qquad \lambda_2 = \mp\, r\sqrt{1+\frac{4I^*}{rK}}.
      $$
      ...
      \begin{bmatrix}
        ...
        -\delta & 0 & 0 \\[6pt]
        \displaystyle \frac{\rho k_1 P(t)^2}{K^2} & \displaystyle -\frac{\rho k_2 P(t)^2}{K^2} & \rho - \frac{2\rho P(t)}{K}
      \end{bmatrix},
      \qquad K = k_1 U(t) - k_2 E(t).
    \]
    $$
    ...
    Los autovalores se obtienen como \(\rho\) y los autovalores de la submatriz \(2\times2\)
    \(
      \begin{pmatrix}0 & -\gamma\\ -\delta & 0
    \end{pmatrix}\), es decir
    \[
      \lambda_1 = \rho, \qquad \lambda_{2,3} = \pm \sqrt{\gamma\delta}.
    \]
    ...
    En este caso, tomamos a \( S(t) \) como una constante.
    ...
    \[
      J =
      \begin{bmatrix}
        \frac{\partial}{\partial S}\left(\frac{dS}{dt}\right) & \frac{\partial}{\partial D}\left(\frac{dS}{dt}\right) \\[8pt]
        \frac{\partial}{\partial S}\left(\frac{dS}{dt}\right) & \frac{\partial}{\partial D}\left(\frac{dS}{dt}\right)
      \end{bmatrix}
    \]
    ...
    \subsubsection{Definición y Búsqueda}

    Un punto de equilibrio $\mathbf{x}^* = (P^*, F^*, W^*)$ satisface:

    \begin{equation}
      \mathbf{f}(\mathbf{x}^*) = \mathbf{0} \quad \Leftrightarrow \quad
      \begin{cases}
        \frac{dP}{dt}\Big|_{\mathbf{x}^*} = 0\\
        \frac{dF}{dt}\Big|_{\mathbf{x}^*} = 0\\
        \frac{dW}{dt}\Big|_{\mathbf{x}^*} = 0
      \end{cases}
    \end{equation}

    \textbf{Método numérico:} Algoritmo de Newton-Raphson multidimensional (función \texttt{fsolve} de SciPy) con múltiples condiciones iniciales:

    \begin{equation}
      \mathbf{x}_0 \in \{(0,0,0), (0.5,0.1,0.5), (1,0.5,1), \ldots, (10,5,10)\}
    \end{equation}

    \textbf{Criterio de convergencia:}
    \begin{equation}
      \max_i |\mathbf{f}_i(\mathbf{x}^*)| < 10^{-6}
    \end{equation}

    \subsubsection{Resultado: Punto de Equilibrio Único}

    El análisis reveló un único punto fijo:

    \begin{equation}
      \mathbf{x}^* = (P^*, F^*, W^*) = (0.000000, 0.000000, 0.000000)
    \end{equation}

    \textbf{Interpretación física:} En el espacio normalizado, este punto representa el \textit{estado promedio histórico} del sistema urbano, no la ausencia de población. Es decir:

    \begin{align}
      P^* = 0 &\quad \Rightarrow \quad P_{\text{real}} = \mu_P \approx 6{,}500{,}000 \text{ habitantes}\\
      F^* = 0 &\quad \Rightarrow \quad \text{Gini}_{\text{real}} = \mu_{\text{Gini}} \approx 0.48\\
      W^* = 0 &\quad \Rightarrow \quad W_{\text{real}} = \mu_W \approx 0.65 \text{ (índice normalizado)}
    \end{align}

    \subsubsection{Linearización: Matriz Jacobiana}

    La estabilidad del punto de equilibrio se determina mediante el Jacobiano evaluado en $\mathbf{x}^*$:

    \begin{equation}
      \mathbf{J}(\mathbf{x}^*) =
      \begin{pmatrix}
        \frac{\partial f_P}{\partial P} & \frac{\partial f_P}{\partial F} & \frac{\partial f_P}{\partial W}\\[0.3cm]
        \frac{\partial f_F}{\partial P} & \frac{\partial f_F}{\partial F} & \frac{\partial f_F}{\partial W}\\[0.3cm]
        \frac{\partial f_W}{\partial P} & \frac{\partial f_W}{\partial F} & \frac{\partial f_W}{\partial W}
      \end{pmatrix}_{\mathbf{x}^*}
    \end{equation}

    \textbf{Cálculo explícito de las derivadas parciales:}

    \begin{align}
      \frac{\partial}{\partial P}\left[rP\left(1-\frac{P}{K}\right) + \gamma_F F\right] &= r - \frac{2rP}{K}\\[0.2cm]
      \frac{\partial}{\partial F}\left[rP\left(1-\frac{P}{K}\right) + \gamma_F F\right] &= \gamma_F\\[0.2cm]
      \frac{\partial}{\partial W}\left[rP\left(1-\frac{P}{K}\right) + \gamma_F F\right] &= 0\\[0.4cm]
      \frac{\partial}{\partial P}\left[-\alpha_F F + \beta_F W\right] &= 0\\[0.2cm]
      \frac{\partial}{\partial F}\left[-\alpha_F F + \beta_F W\right] &= -\alpha_F\\[0.2cm]
      \frac{\partial}{\partial W}\left[-\alpha_F F + \beta_F W\right] &= \beta_F\\[0.4cm]
      \frac{\partial}{\partial P}\left[\alpha_W P + \gamma_W W - \delta_W F\right] &= \alpha_W\\[0.2cm]
      \frac{\partial}{\partial F}\left[\alpha_W P + \gamma_W W - \delta_W F\right] &= -\delta_W\\[0.2cm]
      \frac{\partial}{\partial W}\left[\alpha_W P + \gamma_W W - \delta_W F\right] &= \gamma_W
    \end{align}
    ...
    \begin{equation}
      \mathbf{J}(0,0,0) =
      \begin{pmatrix}
        -5.965 \times 10^{-2} & 8.444 & 0\\
        0 & -4.463 \times 10^{-2} & 2.377 \times 10^{-4}\\
        2.305 \times 10^{-4} & -1.745 \times 10^{-1} & -4.916 \times 10^{-3}
      \end{pmatrix}
    \end{equation}

    \subsubsection{Análisis Espectral}

    Los valores propios de $\mathbf{J}(0,0,0)$ se obtienen resolviendo:
    ...
    \textbf{Teorema de Hartman-Grobman:} Dado que todos los valores propios tienen parte real negativa ($\text{Re}(\lambda_i) < 0$), el punto de equilibrio $(0,0,0)$ es un \textbf{nodo estable asintóticamente} en el sistema linearizado, y por hiperbolicidad, mantiene esta propiedad en el sistema no lineal completo en una vecindad del equilibrio.

    \subsubsection{Interpretación de las Tasas de Decaimiento}

    Los valores propios representan tasas de decaimiento características:

    \begin{align}
      \tau_1 &= -\frac{1}{\lambda_1} \approx 271.3 \text{ unidades de tiempo}\\
      \tau_2 &= -\frac{1}{\lambda_2} \approx 16.8 \text{ unidades de tiempo}\\
      \tau_3 &= -\frac{1}{\lambda_3} \approx 21.8 \text{ unidades de tiempo}
    \end{align}

    \textbf{Interpretación física:}
    \begin{itemize}
      \item El modo más lento ($\lambda_1$) indica que el sistema tarda aproximadamente 271 años en retornar al equilibrio tras perturbaciones en esa dirección
      \item Los modos rápidos ($\lambda_2, \lambda_3$) muestran recuperación en 17-22 años
      \item La disparidad de escalas sugiere \textit{separación de escalas temporales}
    \end{itemize}

    \subsubsection{Interpretación de los Elementos del Jacobiano}

    \begin{equation}
      \mathbf{J} =
      \begin{pmatrix}
        \frac{\partial \dot{P}}{\partial P} & \frac{\partial \dot{P}}{\partial F} & \frac{\partial \dot{P}}{\partial W}\\[0.3cm]
        \frac{\partial \dot{F}}{\partial P} & \frac{\partial \dot{F}}{\partial F} & \frac{\partial \dot{F}}{\partial W}\\[0.3cm]
        \frac{\partial \dot{W}}{\partial P} & \frac{\partial \dot{W}}{\partial F} & \frac{\partial \dot{W}}{\partial W}
      \end{pmatrix} =
      \begin{pmatrix}
        -0.0596 & 8.444 & 0\\
        0 & -0.0446 & 0.000238\\
        0.000231 & -0.1745 & -0.00492
      \end{pmatrix}
    \end{equation}

    \subsubsection{Retroalimentación Desigualdad $t_o$ Población}

    \begin{equation}
      \frac{\partial \dot{P}}{\partial F} = \gamma_F = 8.444
    \end{equation}

    \textbf{Significado:} Un incremento unitario (normalizado) en la desigualdad produce un aumento de 8.44 unidades/tiempo en la tasa de crecimiento poblacional.

    \textbf{Hipótesis explicativa:} Mayor desigualdad genera diferencial de oportunidades económicas $\to$ atracción migratoria hacia centros urbanos.

    \subsubsection{Auto-regulación Poblacional}

    \begin{equation}
      \frac{\partial \dot{P}}{\partial P} = r = -0.0596 < 0
    \end{equation}

    \textbf{Interpretación:} Mecanismo de retroalimentación negativa tipo logístico (efecto de capacidad de carga).

    \textbf{Observación crítica:} El valor negativo de $r$ sugiere que la población está por encima de la capacidad de carga promedio, o que existe emigración neta.

    \subsubsection{Impacto Población $\to$ Bienestar}

    \begin{equation}
      \frac{\partial \dot{W}}{\partial P} = \alpha_W = 2.305 \times 10^{-4}
    \end{equation}

    \textbf{Significado:} Acoplamiento débil. El crecimiento poblacional tiene efecto marginal sobre el bienestar.

    \textbf{Implicación política:} Políticas enfocadas únicamente en crecimiento poblacional tendrán impacto limitado en bienestar.

    \subsubsection{Retroalimentación Negativa del Bienestar}

    \begin{equation}
      \frac{\partial \dot{W}}{\partial W} = \gamma_W = -4.916 \times 10^{-3} < 0
    \end{equation}

    \textbf{Interpretación:} El sistema exhibe \textit{auto-limitación del bienestar}. Aumentos en $W$ generan procesos que tienden a reducirlo.

    \textbf{Mecanismos posibles:}
    \begin{itemize}
      \item Gentrificación (mejoras en bienestar $\to$ aumento de costos $\to$ exclusión)
      \item Saturación de servicios urbanos
      \item Degradación ambiental asociada a desarrollo
    \end{itemize}

    \subsubsection{Construcción del Diagrama de Fase}

    \subsubsection{Proyección Bidimensional}

    Dado que el sistema es tridimensional, visualizamos la dinámica en el plano $(P, W)$ fijando $F = F^* = 0$. El campo vectorial proyectado es:

    \begin{align}
      \frac{dP}{dt}\Big|_{F=0} &= -0.0596 P\left(1 - \frac{P}{6664.04}\right)\\
      \frac{dW}{dt}\Big|_{F=0} &= 2.305 \times 10^{-4} P - 4.916 \times 10^{-3} W
    \end{align}

    \subsubsection{Método de Visualización: Streamplot}

    Se generó una malla rectangular en el espacio $(P, W)$:

    \begin{align}
      P &\in [P^* - 0.4, P^* + 0.4] = [-0.4, 0.4]\\
      W &\in [W^* - 0.4, W^* + 0.4] = [-0.4, 0.4]
    \end{align}

    Para cada punto de la malla $(P_i, W_j)$, se evaluó el campo vectorial:
    ...
    Las líneas de corriente (streamlines) se calculan integrando:

    \begin{equation}
      \frac{dW}{dP} = \frac{\dot{W}(P, W)}{\dot{P}(P, W)}
    \end{equation}

    \subsubsection{Elementos del Diagrama}

    \begin{figure}[H]
      \centering
      \includegraphics[width=0.85\textwidth]{diagrama_fase_P_W.png}
      \caption{Diagrama de fase para el sistema Población (P) vs Bienestar Urbano (W).}
      \label{fig:fase_PW}
    \end{figure}

    \begin{itemize}
      \item \textbf{Punto fijo:} Marcador rojo en $(0, 0)$
      \item \textbf{Streamlines:} Curvas azules mostrando trayectorias en el espacio de fases
      \item \textbf{Flechas:} Indican dirección temporal de evolución del sistema
      \item \textbf{Densidad:} $\rho = 1.8$ líneas por unidad de área
    \end{itemize}

    \subsubsection{Hallazgos Principales}

    \begin{enumerate}
      \item \textbf{Unicidad del equilibrio:} El sistema urbano posee un único punto de equilibrio estable en el espacio de estados normalizado.

      \item \textbf{Estabilidad global local:} Todas las perturbaciones suficientemente pequeñas convergen asintóticamente al estado promedio histórico.

      \item \textbf{Separación de escalas:} Existen dos escalas temporales características:
        \begin{itemize}
          \item Rápida ($\tau \approx 17-22$ años): Ajustes en población y desigualdad
          \item Lenta ($\tau \approx 271$ años): Reconfiguración estructural del bienestar
        \end{itemize}

      \item \textbf{Acoplamiento crítico:} La desigualdad actúa como \textit{amplificador} del crecimiento poblacional ($\partial \dot{P}/\partial F = 8.44$), mientras que el bienestar exhibe auto-limitación ($\partial \dot{W}/\partial W < 0$).
    \end{enumerate}

    \subsubsection{Implicaciones para Política Pública}

    \begin{table}[h]
      \centering
      \begin{tabular}{@{}lp{10cm}@{}}
        \toprule
        \textbf{Hallazgo} & \textbf{Recomendación} \\ \midrule
        $\lambda_{\text{min}} \approx -0.0037$ & Intervenciones estructurales requieren horizontes de planificación de 50+ años \\
        $\partial \dot{P}/\partial F = 8.44$ & Políticas de reducción de desigualdad pueden estabilizar crecimiento urbano descontrolado \\
        $\partial \dot{W}/\partial W < 0$ & Mejoras sostenibles en bienestar requieren modificar parámetros del sistema (ej. capacidad de carga $K$), no solo condiciones iniciales \\
        $U, E$ exógenos & Inversiones en infraestructura ecológica tienen retornos en escalas decadales \\ \bottomrule
      \end{tabular}
      \caption{Traducción de resultados matemáticos a recomendaciones de política}
    \end{table}

    \subsubsection{Limitaciones del Análisis}

    \begin{enumerate}
      \item \textbf{Linearidad local:} El análisis del Jacobiano solo es válido en una vecindad del equilibrio. Perturbaciones grandes pueden exhibir comportamientos no capturados.

      \item \textbf{Forzamiento constante:} Asumir $U$ y $E$ constantes ignora posibles cambios abruptos en uso de suelo o políticas ambientales.

      \item \textbf{Autonomía:} El sistema real puede tener dependencia temporal explícita (tendencias demográficas globales, cambio climático).

      \item \textbf{Datos limitados:} Parámetros estimados con series temporales de 20-30 años pueden no capturar dinámicas de largo plazo.
    \end{enumerate}

    \subsubsection{Extensiones Futuras}

    \begin{itemize}
      \item \textbf{Análisis de bifurcaciones:} Estudiar cómo varían los puntos fijos al modificar parámetros clave (ej. $r$, $K$, $\gamma_F$)

      \item \textbf{Análisis de sensibilidad:} Cuantificar propagación de incertidumbre en parámetros estimados hacia conclusiones sobre estabilidad

      \item \textbf{Extensión espacial:} Incorporar heterogeneidad geográfica mediante ecuaciones de reacción-difusión

      \item \textbf{Validación prospectiva:} Comparar predicciones del modelo con datos de los próximos 5-10 años
    \end{itemize}

    \subsubsection{Apéndice Técnico: Código Python}

    El análisis fue implementado en Python 3.11 utilizando las siguientes librerías:

    \begin{itemize}
      \item \texttt{numpy 1.24}: Álgebra lineal numérica
      \item \texttt{sympy 1.12}: Cálculo simbólico del Jacobiano
      \item \texttt{scipy 1.11}: Resolución de sistemas no lineales (\texttt{fsolve})
      \item \texttt{matplotlib 3.7}: Visualización de diagramas de fase
    \end{itemize}

    \subsubsection{Flujo de Trabajo Computacional}

    \begin{algorithm}[H]
      \caption{Análisis de Estabilidad de Sistemas Dinámicos}
      \begin{algorithmic}[1]
        \STATE Cargar parámetros estimados $\{\alpha_W, \beta_W, \ldots\}$ desde archivo JSON
        \STATE Definir sistema simbólico $\mathbf{f}(\mathbf{x})$ con SymPy
        \STATE Calcular Jacobiano simbólico $\mathbf{J}(\mathbf{x}) = \nabla \mathbf{f}$
        \STATE Convertir a funciones numéricas con \texttt{lambdify}
        \FOR{cada condición inicial $\mathbf{x}_0^{(i)}$}
        \STATE Resolver $\mathbf{f}(\mathbf{x}^*) = 0$ con método de Newton
        \IF{$\|\mathbf{f}(\mathbf{x}^*)\| < 10^{-6}$ y $\mathbf{x}^*$ no duplicado}
        \STATE Agregar $\mathbf{x}^*$ a lista de puntos fijos
        \ENDIF
        \ENDFOR
        \FOR{cada punto fijo $\mathbf{x}^*$}
        \STATE Evaluar $\mathbf{J}(\mathbf{x}^*)$
        \STATE Calcular valores propios $\{\lambda_i\}$
        \STATE Clasificar estabilidad según $\text{sign}(\text{Re}(\lambda_i))$
        \ENDFOR
        \STATE Generar malla $(P_i, W_j)$ en región de interés
        \STATE Evaluar campo vectorial $(\dot{P}, \dot{W})$ en cada nodo
        \STATE Integrar líneas de corriente con algoritmo de Runge-Kutta
        \STATE Superponer punto fijo y etiquetas
        \STATE Exportar diagrama en formato PNG de alta resolución
      \end{algorithmic}
    \end{algorithm}

    \subsubsection{Conclusión General}

    El análisis de estabilidad del sistema de bienestar urbano mediante técnicas de sistemas dinámicos reveló un comportamiento cualitativamente simple pero físicamente significativo: un único atractor global correspondiente al estado promedio histórico de la ciudad. La linearización del sistema en torno a este equilibrio, formalizada a través del Jacobiano, permitió cuantificar las tasas de convergencia y los acoplamientos entre variables de estado.

    Desde una perspectiva de matemática aplicada, este trabajo demuestra la potencia del análisis cualitativo de ecuaciones diferenciales para extraer información sobre sistemas complejos incluso cuando las soluciones analíticas exactas son inaccesibles. La combinación de métodos simbólicos (cálculo del Jacobiano) y numéricos (búsqueda de raíces, integración de trayectorias) constituye un paradigma robusto para el estudio de fenómenos urbanos modelados mediante EDOs.

    La validación de estos resultados requerirá confrontación con datos prospectivos y refinamiento continuo del modelo a medida que se incorporen nuevas observaciones y mecanismos causales.
